...
{
    public partial class VerifyZF10_template : MetroFramework.Forms.MetroForm // Form
    {
       ...
       // Capture _capture;
        ...
        public VerifyZF10_template(FPScanner __fpScanner)
        {
             InitializeComponent();
            Control.CheckForIllegalCrossThreadCalls = false;
           _fpScanner = __fpScanner;
            ...
            {
                //Console.WriteLine(ex.StackTrace);
                AlertForm1 al = new AlertForm1(ex.Message + "\n" + ex.StackTrace, AlertForm1.AlertOptions.OK);
                al.ShowDialog();
               // MessageBox.Show(ex.Message + "\n" + ex.StackTrace, "ERROR");
                //Close();
        ...
        public VerifyZF10_template(FPScanner __fpScanner, sideForm1 ss)
        {
            InitializeComponent();
            Control.CheckForIllegalCrossThreadCalls = false;
            _fpScanner = __fpScanner;
        ...
        void _fpScanner_OnFingerprintsDetected_image(List<Fingerprint> fingerprints)
        {
            #region GUI - Display Fingerprints
            fingerprints_new = fingerprints;
        ...
        async void _fpScanner_OnFingerprintsDetected(List<Fingerprint> fingerprints)
        {
            ...
            string refNO = txt_selectedUser.Text;
            //NewMethod();
            if ((!string.IsNullOrEmpty(refNO)) && (scanBegin))
            {
                try
                {
                    //Verify User
                    ...
                    foreach (Fingerprint fp in fingerprints)
                    {
                        ...
                        string txt_response = await Task.Run(() => Utils.call_WEBAPI_Service("VERIFY_templateFormat", str));
                        // txt_response.Text = await Task.Run(() => Utils.call_WEBAPI_Service("IDENTIFY_templateFormat", str));
                        ...
                        String scoreString = String.Format("{0:0.00}", 0);
                        //if ((bool)dm.result.Hit)
                        if ((bool)dm.found)
                        {
                            ...
                            displayMessage(String.Format("User verified ({0}%), Name={1}, Hand : {2}, Hand Position : {3}",
                                scoreString, dm.ClientName,dm.Hand, dm.HandPosition), Utils.COLOR_DERMALOG_GREEN);
                            ...
                            if ((double)dm.result.Score > maxMatch)
                            {
                                ...
                                aGauge1.Value = (float)maxMatch;
                                CircleProgressbar1.Value = (int)maxMatch;
                                txt_userDetails.Text = "User Details =>" + (string)dm.ClientName;
                                //showAnimation(bunifuCards1);
                                //showAnimation(bunifuCards2);
                                //showAnimation(aGauge1);
                        ...
                        else
                        {
                            ...
                            if (!clientFound)
                            {
                                ...
                                pic_Webcam.Refresh();
                                txt_userDetails.Text = "User not Verified";
                    ...
                    scanBegin = false;
                    displayMessage("Scanning Completed.....", Utils.COLOR_DERMALOG_RED);

                    AlertForm1 al = new AlertForm1("Scanning Completed.....view result", AlertForm1.AlertOptions.OK);
                    al.ShowDialog();
        ...
        private void Start_Capture_selectedUser()
        {
            ...
            if (!string.IsNullOrEmpty(txt_selectedUser.Text))
            {
                ResetGUI();
            ...
            {
                AlertForm1 al = new AlertForm1("Requested user not found, check if the input value is correct...",
        ...
        private void btn_showlarge_Click(object sender, EventArgs e)
        {
            enlargeForm1 fm = new enlargeForm1(pic_Webcam.Image);
            fm.ShowDialog();
        }
    }
}
