...
{
    public partial class EnrollmentWindow_new : MetroFramework.Forms.MetroForm // Form
    {
        ...
        public EnrollmentWindow_new(FPScanner fpScanner, LocalAFIS afis)
        {
        ...
        sideForm1 fm2;
        public EnrollmentWindow_new(FPScanner fpScanner, LocalAFIS afis, string loggedinuser, string enteredUsername, sideForm1 fm)
        {
            Control.CheckForIllegalCrossThreadCalls = false;
            InitializeComponent();
        ...
        public EnrollmentWindow_new(LocalAFIS afis)
        {
            Control.CheckForIllegalCrossThreadCalls = false;
            InitializeComponent();
        ...
        private void BindFPScannerEvents()
        {
            ...
            _fpScanner.OnFingerprintsDetected += _fpScanner_OnFingerprintsDetected_e;
        }
        ...
        void _fpScanner_OnFingerprintsDetected_e(List<Fingerprint> fingerprints)
        {
            _fpScanner.StopCapturing();
            ...
            switch (selected_fingers)
            {
                case "left":
                    ...
                    {
                        IAsyncResult result = fm2.enrollmentWindow.BeginInvoke((MethodInvoker)delegate ()
                        //IAsyncResult result = this.BeginInvoke((MethodInvoker)delegate ()

                        //this.BeginInvoke((MethodInvoker)delegate ()
                        //this.Invoke((MethodInvoker)delegate ()
                        //fm2.enrollmentWindow.BeginInvoke((MethodInvoker)delegate ()
                        {
        ...
        private void  call_ExternalAPI(dynamic dd)
        {
            ...
            dd.others = "Anything";
            pic_Webcam.Refresh();
            dd.capture_picture = pic_Webcam==null ? "": Utils.ImageToBase64(pic_Webcam, true);
            dd.date = DateTime.Now.ToString("MMMM dd, yyyy"); ;
            dd.ExternalAPI = Properties.Settings.Default.ExternalAPIAddress;
            ...
            string APIHandle = "save_ENROLLFEEDBACK";   // @"submit_Online_Request";
            ...
            //else
            {
                ...
                df = Newtonsoft.Json.JsonConvert.DeserializeObject<dynamic>(resp);
        ...
        private bool Identify_AfterCapture(List<Fingerprint> capturedfingerprints)
        {
            ...
            Task.Run(() =>
            {
                
                {
                    ...
                    strResponse = Utils.call_WEBAPI_Service("IDENTIFY_List_templateFormat", strBody);
                }
